...
async function kvGetDay(dayIndex: number): Promise<DayEntry | null> {
  try {
    ...
    if (needsNewContract) {
      // For days after Jan 15, 2026, ONLY use new contract cache - never fall back to legacy
      const newKey = await client.get(kvKey(dayIndex));
      return newKey as DayEntry | null;
    } else {
      // For days before Jan 15, 2026, try both keys (legacy might have data)
            ...
            for (let j = 0; j < CONC && (newContractStartDay + i + j) <= newContractEndDay; j++) {
              ...
              slice.push((async () => {
                ...
                try {
                  ...
                  console.error(`Failed to fetch day ${d}`, { error: err?.message || String(err), dayStart, dayEnd });
                  return [] as Row[];
                }
              })());
            }
